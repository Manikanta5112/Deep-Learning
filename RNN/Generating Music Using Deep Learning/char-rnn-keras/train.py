    ...
    for start in range(0, batch_chars - SEQ_LENGTH, SEQ_LENGTH): # (0, 8040, 64)
        ...
        for batch_idx in range(0, BATCH_SIZE): # (0,16)
            for i in range(0, SEQ_LENGTH): #(0,64)
                X[batch_idx, i] = T[batch_chars * batch_idx + start + i] # just assigning the numeric value at that index of a full text T
                Y[batch_idx, i, T[batch_chars * batch_idx + start + i + 1]] = 1
        yield X, Y # using yield for memory efficient purpose (it will return each (X,Y) immediately)
...
def train(text, epochs=100, save_freq=10):
    ...
    for epoch in range(epochs):
        ...
        # for specified frequency storing the model
